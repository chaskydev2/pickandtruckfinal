...
@section('content')
    <div class="container py-4">
        ...
        <ul class="nav nav-tabs mb-4">
            <li class="nav-item">
                <a class="nav-link {{ !request()->has('view') ? 'active' : '' }}" href="{{ route('ofertas_carga.index') }}">
            ...
            <div>
                ...
                <p class="text-muted">
                    {{ request()->get('view') === 'mine'
                        ? 'Administra tus ofertas de carga publicadas'
                        : 'Explora las últimas publicaciones en el mercado' }}
                </p>
            </div>
        ...
        <div class="card mb-4">
            <div class="card-body">
                <form action="{{ route('ofertas_carga.index') }}" method="GET" class="row g-3 align-items-end">
                    @if (request()->has('view'))
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select" id="tipo_carga" name="tipo_carga">
                            <option value="">Todos los tipos</option>
                            @foreach ($cargoTypes as $type)
                                <option value="{{ $type->id }}"
                                    {{ request('tipo_carga') == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
        ...
        @if (count($ofertas) > 0)
            ...
            <div id="publications-list" class="timeline-feed">
                @foreach ($ofertas as $oferta)
                    @include('partials.oferta_carga_card', ['oferta' => $oferta])
                @endforeach
            </div>
        @else
            <div class="card border-0 shadow-sm">
                <div class="card-body text-center py-5">
                    <div class="display-1 text-muted mb-3">
                        <i class="fas {{ request()->get('view') === 'mine' ? 'fa-box-open' : 'fa-search' }}"></i>
                    </div>
                    <h3 class="text-muted mb-3">
                        {{ request()->get('view') === 'mine'
                            ? 'No has publicado ninguna oferta de carga aún'
                            : 'No hay ofertas de carga disponibles' }}
                    </h3>
    ...
    @push('styles')
        ...
        <style>
            ...
            .nav-tabs .nav-link {
                color: #1a2b4c;
                /* Azul oscuro para pestañas inactivas */
                border: none;
